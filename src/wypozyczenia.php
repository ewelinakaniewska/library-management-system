...
<html lang="en">
  ...
  <div class="container " >
        <h2 class="mt-4" id="cennik">Wypożyczenia</h2>
        <a href="insertWypozyczenie.php"><button class="btn btn-outline-success" >Dodaj nowe wypożyczenie</button></a>
        ...
        <tbody>
            ...
            try {
                ...
                while ($row = oci_fetch_assoc($result)) {
                    ...
                    echo "<td>" . htmlentities($row['ZAMOWIENIEID'], ENT_QUOTES) . "</td>\n";
                    echo "<td>" . htmlentities($row['PRACOWNIKID'], ENT_QUOTES) . "</td>\n";
                    echo "<td>" . htmlentities($row['DATAWYPOZYCZENIA'], ENT_QUOTES) . "</td>\n";
                    echo "<td>" . htmlentities($row['DATAZWROTU'], ENT_QUOTES) . "</td>\n";
                    echo "<td><form action='updateWypozyczenie.php' method='post'><input type='hidden' name='update' value='" . htmlentities($row['ID'], ENT_QUOTES) . "'><input type='submit' class='btn btn-outline-success' value='Edytuj'></form></td>\n";
                    echo "<td><form action='wypozyczenia.php' method='post'><input type='hidden' name='delete' value='" . htmlentities($row['ID'], ENT_QUOTES) . "'><input type='submit' class='btn btn-outline-danger' value='Usuń'></form></td>\n";
                    echo "</tr>\n";
                }
  ...
  <?php
    ...
    function func_delete()
    {
      require('connection.php');
        ...
        $sql = "BEGIN wypozyczenie_pkg.DELETE_WYPOZYCZENIE(:id); END;";
        $stmt = oci_parse($conn, $sql);
