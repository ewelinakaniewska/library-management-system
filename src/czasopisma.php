...
<div id="inne" class="container mt-5 mb-3">
    <div class="container">
        <h2 class="mt-4" id="cennik">Czasopisma</h2>
        <a href="insertCzasopismo.php"><button class="btn btn-outline-success">Dodaj nowe czasopismo</button></a>
            ...
            <table class="table table-bordered">
                ...
                try {
                    ...
                    while ($row = oci_fetch_assoc($result)) {
                        ...
                        echo "<td>" . htmlentities($row['WYDAWCA'], ENT_QUOTES) . "</td>\n";
                        echo "<td>" . htmlentities($row['CZYTELNIAID'], ENT_QUOTES) . "</td>\n";
                        echo "<td>" . htmlentities($row['STATUS'], ENT_QUOTES) . "</td>\n";
                        echo "<td>" . htmlentities($row['NUMERWYDANIA'], ENT_QUOTES) . "</td>\n";
                        echo "<td><form action='updateCzasopismo.php' method='post'><input type='hidden' name='update' value='" . htmlentities($row['ID'], ENT_QUOTES) . "'><input type='submit' class='btn btn-outline-success' value='Edytuj'></form></td>\n";
                        echo "<td><form action='czasopisma.php' method='post'><input type='hidden' name='delete' value='" . htmlentities($row['ID'], ENT_QUOTES) . "'><input type='submit' class='btn btn-outline-danger' value='Usuń'></form></td>\n";
                        echo "</tr>\n";
                    }
    ...
    function func_delete()
    {
        ...
        $sql = "BEGIN czasopismo_pkg.DELETE_CZASOPISMO(:id); END;";    
        $stmt = oci_parse($conn, $sql);
