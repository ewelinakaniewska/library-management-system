...
<html lang="en">
  ...
  <div class="container " >
        <h2 class="mt-4" id="cennik">Czytelnicy</h2>
        <a href="insertCzytelnik.php"><button class="btn btn-outline-success" >Dodaj nowego czytelnika</button></a>
        ...
        <tbody>
            ...
            try {
                ...
                while ($row = oci_fetch_assoc($result)) {
                    ...
                    echo "<td>" . htmlentities($row['ADRESZAMIESZKANIA'], ENT_QUOTES) . "</td>\n";
                    echo "<td>" . htmlentities($row['DATAURODZENIA'], ENT_QUOTES) . "</td>\n";
                    echo "<td>" . htmlentities($row['DATAZAPISANIA'], ENT_QUOTES) . "</td>\n";
                    echo "<td>" . htmlentities($row['RANGA'], ENT_QUOTES) . "</td>\n";
                    echo "<td><form action='updateCzytelnik.php' method='post'><input type='hidden' name='update' value='" . htmlentities($row['ID'], ENT_QUOTES) . "'><input type='submit' class='btn btn-outline-success' value='Edytuj'></form></td>\n";
                    echo "<td><form action='czytelnicy.php' method='post'><input type='hidden' name='delete' value='" . htmlentities($row['ID'], ENT_QUOTES) . "'><input type='submit' class='btn btn-outline-danger' value='Usuń'></form></td>\n";
                    echo "</tr>\n";
                }
  ...
  <?php
    ...
    function func_delete()
    {
      require('connection.php');
        ...
        $sql = "BEGIN czytelnik_pkg.DELETE_CZYTELNIK(:id); END;";
        $stmt = oci_parse($conn, $sql);
