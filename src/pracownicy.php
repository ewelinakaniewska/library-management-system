...
<body>
    ...
    <div id="inne" class="container mt-5 mb-3">
        <div class="container">
            <h2 class="mt-4" id="pracownicy">Pracownicy</h2>
            <a href="insertPracownik.php"><button class="btn btn-outline-success">Dodaj nowego pracownika</button></a>
                    ...
                    <tbody>
                        ...
                        try {
                            ...
                            while ($row = oci_fetch_assoc($result)) {
                                ...
                                echo "<td>" . htmlentities($row['STANOWISKO'], ENT_QUOTES) . "</td>\n";
                                echo "<td>" . htmlentities($row['CZYTELNIAID'], ENT_QUOTES) . "</td>\n";
                                echo "<td>" . htmlentities($row['PENSJA'], ENT_QUOTES) . "</td>\n";
                                echo "<td>" . htmlentities($row['DATAZATRUDNIENIA'], ENT_QUOTES) . "</td>\n";
                                echo "<td><form action='updatePracownik.php' method='post'><input type='hidden' name='update' value='" . htmlentities($row['ID'], ENT_QUOTES) . "'><input type='submit' class='btn btn-outline-success' value='Edytuj'></form></td>\n";
                                echo "<td><form action='pracownicy.php' method='post'><input type='hidden' name='delete' value='" . htmlentities($row['ID'], ENT_QUOTES) . "'><input type='submit' class='btn btn-outline-danger' value='Usuń'></form></td>\n";
                                echo "</tr>\n";
                            }
   ...
   function func_delete()
   {
     require('connection.php');
       ...
       $sql = "BEGIN pracownik_pkg.DELETE_pracownik(:id); END;";
       $stmt = oci_parse($conn, $sql);
