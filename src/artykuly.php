...
<html lang="en">
  ...
  <body>
    ...
    <div id="inne" class="container mt-5 mb-3">
      <div class="container">
        <h2 class="mt-4" id="cennik">Artykuły</h2>
        <a href="insertArtykul.php"><button class="btn btn-outline-success">Dodaj nowy artykuł</button></a>
            ...
            <tbody>
              ...
              try {
                ...
                while ($row = oci_fetch_assoc($result)) {
                  ...
                  echo "<td>" . htmlentities($row['TYTUL'], ENT_QUOTES) . "</td>\n";
                  echo "<td>" . htmlentities($row['AUTOR'], ENT_QUOTES) . "</td>\n";
                  echo "<td><form action='updateArtykul.php' method='post'><input type='hidden' name='update' value='" . htmlentities($row['ID'], ENT_QUOTES) . "'><input type='submit' class='btn btn-outline-success' value='Edytuj'></form></td>\n";
                  echo "<td><form action='artykuly.php' method='post'><input type='hidden' name='delete' value='" . htmlentities($row['ID'], ENT_QUOTES) . "'><input type='submit' class='btn btn-outline-danger' value='Usuń'></form></td>\n";
                  echo "</tr>\n";
                }
    ...
    function func_delete() {
      ...
      $sql = "BEGIN artykul_pkg.DELETE_ARTYKUL(:id); END;";
      $stmt = oci_parse($conn, $sql);
